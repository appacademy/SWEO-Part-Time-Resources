...
function restoreTheme() {
	const splitCookiesOnSemicolon = document.cookie.split("; ")
    const cookies = splitCookiesOnSemicolon.map(cookie => cookie.split("="))
    cookies.forEach(cookie => {
        if (cookie[0] === "themeName") setTheme(cookie[1])
    })
}
...
function restoreName() {
    ...
    const cookies = splitCookiesOnSemicolon.map(cookie => cookie.split("="))
    cookies.forEach(cookie => {
        if (cookie[0] === "name") setInputValue("display-name", cookie[1])
    })
}
