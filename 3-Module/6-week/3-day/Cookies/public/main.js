...
function storeTheme(themeName) {
  document.cookie = `theme=${themeName}; max-age=32000000`;
}

// For restoring theme from cookies, if selected by the user in the past
function restoreTheme() {
  //? If you have more than one cookie

  const cookieArray = document.cookie.split('; ');
  const themeCookie = cookieArray.find((el) => el.includes('theme'));

  if (themeCookie) {
    const themeName = themeCookie.split('=')[1];
    setTheme(themeName);
  }
  //! If you only have one cookie
  // console.log(document.cookie) // 'theme=wizard'
  // console.log(document.cookie.split("=")) // ["theme", "wizard"]
  // console.log(document.cookie.split("=")[1]) "wizard"
}
...
function restoreName() {
  const cookieArray = document.cookie.split('; ');
  const nameCookie = cookieArray.find((el) => el.includes('name'));
