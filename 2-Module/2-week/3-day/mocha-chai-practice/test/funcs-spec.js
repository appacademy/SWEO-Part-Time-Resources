...
describe("isFive(num)", () => {
  it("should return true if the num is 5 otherwise false", () => {
    ...
    const actual1 = isFive(num1); // should be true //isFive(5) ==> true?

    // expect(actual1).to.be.true; //assert that the actual asnwer <isFive(5)> is giving the expect answer (`true`)
    
    // expect(actual1).true; //we can remove the .to.be because they are simple properties to help represent english words for more clarity 
    
    // expect(actual1).have.be.to.which.been.true; //this works, but is not readable.
