...
function helperTraversal(root, res, i) {
  if (!root) return;

  if (i === res.length) res.push(root.value);

  res[i] = Math.max(res[i], root.value);
  // console.log(res, { i }, root.value);
  helperTraversal(root.left, res, i + 1);
  helperTraversal(root.right, res, i + 1);
}

function findMaxEachLevel(root) {
  let res = [];

  helperTraversal(root, res, 0);
