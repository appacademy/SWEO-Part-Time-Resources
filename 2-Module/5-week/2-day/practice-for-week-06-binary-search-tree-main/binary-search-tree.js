...
class BinarySearchTree {
  ...
  search(val) {
    //lets create a currentNode that will be a POINTER to traverse the tree for us

    //while currentNode is not null
    //  1) check if the currentNode's value is equal to the target val, return true if so.

    //  2) else if the target is less than the currentNode's value, reassign currentNode to be the currentNode.left

    //  3) else if the target is greater than the currentNode's value, reassign currentNode to be the currentNode.right

    
    //after the while loop, if the while loop didn't find any match, return false by default.
  }
