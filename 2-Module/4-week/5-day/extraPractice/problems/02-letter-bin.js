...
class LetterBin {
    constructor() {
        ...
        this.letters = []; // since there are no nodes for us to use and no instructions to do differently, an array will work fine here. 
        this.length = 0; // added the length to keep numLetters at O(1) time.
    }
    ...
    addLetter(message) {
        ...
        this.letters.push(letter); // top of the stack = back of the array
        // could be O(n) time since they didn't give us a number of spots to reserve in memory :(
        this.length++;
        return letter;
    }
    ...
    removeLetter() {
        ...
        const letter = this.letters.pop(); // O(1) time
        this.length--;
        return letter;
    }
}
