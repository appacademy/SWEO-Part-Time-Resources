...
//1) True||False: Arrays can store different data types 
    //answer: FALSE. Arrays (static and dynamic) hold similar datatypes. In javascript arrays aren't holding different data. They are holding memory addresses called `pointers`. Pointers will provide the real location of the different data.
...
//3) Given the array above, in an 8-bit system, binary value or string of the given decimal value of 114? (convert this decimal to binary)
    //answer: 1,2,4,8,16,32,64,128  -->  01110010
...
//- what is the value Address of the last element `255` in that array (hint: what is 255's index)
    //ANSWER: 206. O(1) access time because we can use math to get the address given if the size of the data are all the same (always know that its x amount of bytes that each number takes up) and that the elemtns are stored in continuous sequence.
