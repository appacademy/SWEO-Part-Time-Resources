...
objects with `id` and `count` properties as its values.
...
store file (__src/store/store.js__). Use the `produceReducer` as an example if
...
**Hint:** To create a new object that includes all the keys of another object
...
Test this out by navigating to [http://localhost:5173] and clicking the plus
...
**Note:** If you open the cart by clicking the "Checkout" button right now, you
will still not see the produce that you have added. That's because you haven't
...
If you refresh [http://localhost:5173] and add a produce to the cart, you
should now see something in the cart!
