...
describe("04 - Display a Book's Details", () => {
    ...
    describe('show a detailed book page when the link to "/books/:bookId" is clicked on the BookIndexItem', () => {
        it('should show the first book when the "Book #1" text link is clicked', async () => {
            ...
            expect(container.innerHTML).toBe(
                '<h1>aA Lending Library</h1><section>ID: 1<br>Title: The Count of Monte Cristo<br>Author: Alexandre Dumas<br><button>Check Out</button><br><a href="/">Back to Books List</a></section>'
            );
        });
        ...
        it('should show the fourth book when the "Book #4" text link is clicked', async () => {
            ...
            await waitForElementToBeRemoved(link);
            expect(container.innerHTML).toBe(
                '<h1>aA Lending Library</h1><section>ID: 4<br>Title: Harry Potter and the Philosopher\'s Stone<br>Author: J.K. Rowling<br><button>Check Out</button><br><a href="/">Back to Books List</a></section>'
            );
        });
    });
    ...
    describe('check out the book when "Check Out" button is clicked on the BookShow', () => {
        it('should change the "Check Out" button to become a "Return" button when clicked', async () => {
            ...
            expect(container.innerHTML).toBe(
                '<h1>aA Lending Library</h1><section>ID: 1<br>Title: The Count of Monte Cristo<br>Author: Alexandre Dumas<br><button>Return</button><br><a href="/">Back to Books List</a></section>'
            );
        });
    });
