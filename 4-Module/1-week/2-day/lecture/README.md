...
## What is Express?
Express is a minimal and flexible Node.js backend framework that is used for building APIs and web applications. It is easy to use, has efficient routing, and convenient debugging.

Backend frameworks are the libraries of server-side programming languages that help build the backend structure for a website. Frameworks for web development are incredibly helpful in constructing interactive applications.
...
- res.send() - send HTTP response, Content-Type res header is auto set depending on argument passed in. For example, if an array or an pojo is passed in, it will set the content headers to json.
- res.json() - send JSON response. This will call the JSON.stringify method on the argument and set the content headers to application/json, then send your response.
