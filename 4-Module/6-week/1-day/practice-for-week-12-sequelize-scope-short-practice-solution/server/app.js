...
app.get('/instruments/:instrumentType', async (req, res, next) => {
    const { instrumentType } = req.params;
    console.log(req.params)
    const keyboards = await Instrument.scope({method: ['instrumentType', instrumentType]}).findAll()
    res.json(keyboards);
});


// app.get('/instruments/string', async (req, res, next) => {
//     const strings = await Instrument.scope('instrumentType', 'string').findAll()
//     res.json(strings);
// });

// app.get('/instruments/woodwind', async (req, res, next) => {
//     const woodWinds = await Instrument.scope('instrumentType', 'woodwind').findAll()
//     res.json(woodWinds);
// });
...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
    ...
    const filterStoreInstruments = await Instrument.scope({method : ['atStore', storeId]}).findAll()
    // Your code here
