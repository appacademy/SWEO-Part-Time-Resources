...
app.post('/bands/:bandId/musicians', async (req, res, next) => {
    ...
    // find the band that we want by id
    const bandMatch = await Band.findByPk(bandId);
    // create the new musician and add it to the band it belongs to
    const newMusician = await bandMatch.createMusician({firstName,lastName});

    return res.json({
        message: `Created new musician for band ${bandMatch.name}`,
...
app.post('/musicians/:musicianId/instruments', async (req, res, next) => {
    ...
    const {instrumentIds} = req.body //[1, 2, etc.]

    // find the musician, then addInstrument to the selected musician
    const musicianMatch = await Musician.findByPk(musicianId);
    // console.log("Musician Match:", musicianMatch)
    await musicianMatch.addInstrument(instrumentIds)

    res.json({
        message: `Associated ${musicianMatch.firstName} with instruments ${instrumentIds}.`
    })
