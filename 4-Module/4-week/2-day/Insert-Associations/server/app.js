...
app.post('/bands/:bandId/musicians', async (req, res, next) => {
  ...
  if (!firstName || !firstName.length || !lastName || !lastName.length) {
    return res.status(404).json({
      message: 'Please first and last name ya dingus',
    });
  }
  ...
  res.json({
    message: `Created new musician named ${firstName} for the band ${chosenBand.name}`,
...
app.post('/musicians/:musicianId/instruments', async (req, res, next) => {
  ...
  const instrumentsArray = req.body.instrumentIds;

  if (!instrumentsArray || !instrumentsArray.length) {
    return res.status(404).json({
      message: 'Please instruments please ya dongus',
    });
  }

  const chosenMusician = await Musician.findByPk(id);

  chosenMusician.addInstruments(instrumentsArray);

  res.json({
    message: `We did it correctly ${instrumentsArray}`,
  });
});
