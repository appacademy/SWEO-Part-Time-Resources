...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    for(let i = 0; i < musicianInstruments.length; i++){
      ...
      // console.log(musician, 'line 52222222')
      const foundMusician = await Musician.findOne({where: musician})
      const foundInstruments = await Instrument.findAll({where: {[Op.or]: instruments}})
