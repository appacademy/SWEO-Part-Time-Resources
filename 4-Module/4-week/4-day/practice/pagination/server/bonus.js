...
app.get('/musicians', async (req, res, next) => {
    ...
    let size = Number(queryString.size);
    // if page and size is anything but a number, then set default
    ...
    let limit;
    let offset;
    // if page is 0, then get all matches
    if (page === 0) {
        limit = null; //if limit is -1 it will work too
        offset = 0;
    }
    // otherwise calculate the limit and offset values
    else {
        limit = size;
        offset = size * (page - 1)
    }
