...
app.get('/musicians', async (req, res, next) => {
    ...
    const firstNameData = req.query.firstName
    // console.log("firstNAme", firstName)
    if(firstNameData) query.where.firstName = firstNameData
    // Your code here
    
    // Add keys to the WHERE clause to match the lastName param, if it exists.
    // End result: { where: { lastName: req.query.lastName } }
    
    const lastNameData = req.query.lastName
    // console.log("firstNAme", firstName)
    if(lastNameData) query.where.lastName = lastNameData
    ...
    const bandNameData = req.query.bandName;
    if(bandNameData){
        query.include.push({
            model: Band,
            where: {
    ...
    */
    ...
    const instrumentTypesArr = req.query.instrumentTypes
    console.log(instrumentTypesArr);

    if(instrumentTypesArr && Array.isArray(instrumentTypesArr)){
        query.include.push({
            model: Instrument,
            attributes: ["id", "type"],
            where:{
                // type is a column on the Instrument table. If the record we are getting has a column with a name type and the data inside that column is one of the item in instrumentTypesArr, then include that record
                type: instrumentTypesArr
            },
            through:{
                attributes: []
            }
