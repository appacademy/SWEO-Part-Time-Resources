...
app.get('/cats/:id/toys', async (req, res, next) => {
  const catToysAggregateData = await Cat.findByPk(req.params.id, {
    ...
    attributes: [
      ...
      // key of `toyCount`
      //! Since id is available on Toys and Cats, I specify the id I'm counting is from Toys
      ...
      //! Since price only exists on Toys, I don't need to specify the table
...
app.get('/data-summary', async (req, res, next) => {
  ...
  const toySummary = await Toy.findAll({
    attributes: [
      ...
      [sequelize.fn('TOTAL', sequelize.col('price')), 'totalPriceOfAllToys'],
