...
const { DraftPick } = require('../db/models');
...
describe('Phase 4 Specs - Fans', () => {
  ...
  describe('DELETE /fans/:fanId', () => {
    let id = 2;
    it('deletes the Fan specified by id', async () => {
      await chai.request(server)
        .delete(`/fans/${id}`)
        .then((res) => {
          expect(res).to.have.status(200);
    ...
    it('cascade deletes the DraftPicks associated with the Fan specified by id', async () => {
