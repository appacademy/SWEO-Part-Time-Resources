...
student_id, person_ssn, car_vin

one-to-one   : one row in table A references only one row in table B
one-to-many  : one row in table A references many rows in table B
many-to-many : one row in table A references many rows in table B
--             and
--             one row in table B references many rows in table A
*/
.mode box
.headers on
...
SELECT students.id, name, juice FROM students
JOIN juice_boxes ON students.id = student_id;
