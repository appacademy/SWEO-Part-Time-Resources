...
// setTimeout, takes a callback (first arg) and then executes the callback after the 
// given delay (second arg), and pass to the callback any optional arguments (third+ args)
...
// Remember that the call stack is only empty, after EVERY line of synchronous 
// code in the file has been executed and evaluated.

// after the callstack is empty (no more sync code to run), EVENT LOOP dequeue 
// the first thing in the message queue and pushes it into the main call stack 
// to be evaluated.
...
//the third+ argument passed to setTimeout are the arguments that you want to 
// invoke the callback(first arg) with.

//remember we pass a function to setTimeout UNINVOKED. then setTimeout does the 
// heavy lifting for us, and invokes the callback func that we pass to it, with 
...
//setTimeout will invoke the anonymous wrapper function after the given 
// delay --> execute all the line of code inside that wrapper.
