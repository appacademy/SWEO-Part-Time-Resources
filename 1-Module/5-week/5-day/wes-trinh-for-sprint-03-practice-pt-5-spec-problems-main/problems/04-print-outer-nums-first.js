...
function printOuterNumsFirst(nums) {
  ...
  let num;
  if (nums.length % 2 === 0) { //if nums.length is even, we should hit this block
    num = nums.shift(); //this line should remove from the front and log it out.
  } else { //if nums.length is odd, expect to hit this block here
    num = nums.pop();
  }
