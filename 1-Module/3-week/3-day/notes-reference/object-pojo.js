console.log("----------Plain Old Javascript Object----------");
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Objects-Pojos
*/


console.log("----------Defining an Object----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Defining-an-Object
*/

//avoid passively reading! Try, test / paste your code or examples here

console.log("----------setting / updating or creating data in an object----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Setting-keyvalue
*/

//avoid passively reading! Try, test / paste your code or examples here

console.log("----------getting / reading data in an object----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Getting-values
*/


console.log("----------Checks if Key Exists----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Checks-if-Key-Exists
*/


console.log("----------Iterate Through Objects----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Iterate-Through-Objects
*/


console.log("----------Arrays vs. Objects----------")
/*NOTES LINK (ctrl + left-click to open): 
https://hackmd.io/ibCTcNIERy2irC8OUTRcFQ?view#Arrays-vs-Objects
*/

